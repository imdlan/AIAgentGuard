...
### Fixed
- **IPv6 Compatibility**: Fixed network address format for IPv6 support in host checking
  - Use `net.JoinHostPort` instead of manual string formatting
  - Properly handles IPv6 addresses in `CheckSpecificHost` function
- **Code Quality**: Removed duplicate code blocks in webui/backend/main.go
  - Deleted duplicate API route definitions
  - Removed duplicate "suid" field in getScannerStatus()
- **Panic Misuse**: Fixed improper panic in metrics collector
  - Changed from `panic(err)` to log output in StartMetricsServerAsync
  - Prevents unexpected crashes in goroutine

### Changed
- **Dependency Cleanup**: Removed 30+ unused dependencies from go.mod
  - Smaller binary size and reduced attack surface
...
### Fixed
- **Installation Script**: Added fallback method for GitHub API rate limiting
  - Alternative version detection using HTTP redirect headers
  - Better error messages with manual installation instructions

### Added
- **Version Command**: Added `agent-guard version` command to display current version
  - Shows version information embedded in binary during build
  - Useful for troubleshooting and support

### Changed
- Moved version variable from main.go to cmd package for better organization
- Updated Goreleaser ldflags to use `cmd.version` instead of `main.version`
...
### Added
- **Multi-Language Support (i18n)**: Auto-detect system language for output
  - Automatic language detection from LANG/LC_ALL environment variables
  - macOS system preferences detection (AppleLocale, AppleLanguages)
